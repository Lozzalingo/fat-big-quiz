...
async function main() {
  ...
  if (!dryRun && totalImported > 0) {
    ...
    if (hasApiKey) {
      console.log(`\n[History] Running LLM classification on ${totalImported} newly imported questions...`);
      try {
        ...
        console.log(`[History] Classification done: ${classifyStats.classified} classified, ${classifyStats.flagged} flagged, ${classifyStats.errors} errors`);
      } catch (err) {
        console.error("[History] Post-scrape classification error:", err.message);
      }
    } else {
      console.log("\n[History] No LLM API key configured - skipping post-scrape classification");
      console.log("[History] Run manually: node server/scripts/classifyQuestions.js");
    }
  }
}
