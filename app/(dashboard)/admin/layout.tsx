...
export default function FBQAdminLayout({
  ...
  // Resolve admin auth token (localhost shortcut or NextAuth JWT)
  useEffect(() => {
    const isLocalhost =
    ...
    if (isLocalhost) {
      console.log("[AdminLayout] Localhost detected - using localhost auth");
      setAdminSecret("localhost");
      return;
    }

    console.log("[AdminLayout] Fetching auth token for production");
    fetch("/api/auth/token")
      .then((res) => res.json())
      .then((data) => {
        if (data.token) {
          setAdminSecret(data.token);
          console.log("[AdminLayout] Auth token received");
        } else {
          console.error("[AdminLayout] No token returned:", data.error);
        }
      })
      .catch((err) => {
        console.error("[AdminLayout] Token fetch error:", err);
      });
  }, []);

  // Connect socket.io for auto-save (used by AdminEventsPage)
  useEffect(() => {
    const s = io(API_BASE, { transports: ["websocket", "polling"] });
    s.on("connect", () => console.log("[AdminLayout] Socket connected"));
    s.on("disconnect", () => console.log("[AdminLayout] Socket disconnected"));
    setSocket(s);
    return () => {
      s.disconnect();
    };
